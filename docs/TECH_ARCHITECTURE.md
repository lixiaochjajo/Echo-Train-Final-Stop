...
## 8. Milestone 0.2 实现记录

本轮已完成：

- 存档结构升级到版本 2，并兼容迁移版本 1
- 死亡后生成阻塞式遗产选择
- 装备铭刻与确定性命运抽取
- 抽取权重纳入装备价值、敌人威胁与玩家强度差
- 战斗模拟保持纯函数，可同时用于预估和实际结算
- 战斗结果增加结构化 `steps`，供 UI 与 t3d 动画共同消费
- 精英通过 `enemyMemory` 永久记录上一轮战术
- 维修隔离门作为多条件地图阻挡事件
- 渲染层增加路径高亮、战斗缩放反馈和 context lost 恢复
- 增加不依赖 DOM / GPU 的状态冒烟测试

下一轮优先把地图从单个字符模板升级为 `carriages` 数据集合，避免多车厢继续堆叠条件分支。
